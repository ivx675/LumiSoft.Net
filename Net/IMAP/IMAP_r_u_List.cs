    ...
    public class IMAP_r_u_List : IMAP_r_u
    {
        ...
        public static IMAP_r_u_List Parse(string listResponse)
        {
            ...
            */
            ...
            if(delimiter == null){
                throw new ArgumentException("Invalid LIST response, missing hierarchy delimiter.");
            }

            string? folder = r.ReadToEnd()?.Trim();
            if(folder == null){
                throw new ArithmeticException("Invalid LIST response, missing folder name.");
            }
            folder = IMAP_Utils.DecodeMailbox(folder);

            return new IMAP_r_u_List(folder,delimiter[0],attributes.Split(' '));
        }
