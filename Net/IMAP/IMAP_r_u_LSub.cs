    ...
    public class IMAP_r_u_LSub : IMAP_r_u
    {
        ...
        public static IMAP_r_u_LSub Parse(string lSubResponse)
        {
            ...
            */
            ...
            if(delimiter == null){
                throw new ArgumentException("Invalid LSUB response. hierarchy-delimiter is missing.");
            }

            string? folder = r.ReadToEnd()?.Trim();
            if(folder == null){
                throw new ArgumentException("Invalid LSUB response. name is missing.");
            }
            folder = TextUtils.UnQuoteString(IMAP_Utils.DecodeMailbox(folder));
