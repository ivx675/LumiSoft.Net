    ...
    public class IMAP_r_u_ListRights : IMAP_r_u
    {
        ...
        public static IMAP_r_u_ListRights Parse(string listRightsResponse)
        {
            ...
            */
            ...
            string? folder = r.ReadWord(true);//IMAP_Utils.Decode_IMAP_UTF7_String(r.ReadWord(true));
            if(folder == null){
                throw new ArgumentException("Invalid LISTRIGHTS response value, mailbox-name is missing.","listRightsResponse");
            }
            folder = IMAP_Utils.Decode_IMAP_UTF7_String(folder);
            string? identifier = r.ReadWord(true);
            if(identifier == null){
                throw new ArgumentException("Invalid LISTRIGHTS response value, user/gruop identifier is missing.","listRightsResponse");
            }
            string? reqRights  = r.ReadWord(true);
            string? optRights  = r.ReadToEnd();
