	...
	public class FTP_Client : TCP_Client
    {
        ...
        private class DataConnection : IDisposable
        {
            ...
            private void CreateSocket()
            {
                ...
                ArgumentNullException.ThrowIfNull(m_pOwner.LocalEndPoint);
            ...
            {
                get{ 
                    if(m_pSocket != null && m_pSocket.LocalEndPoint != null){
                        return (IPEndPoint)m_pSocket.LocalEndPoint;
                    }
                    else{
                        return null;
                    }
        ...
        public void Abort()
        {
			...
			if(line == null || !line.StartsWith("2")){
				throw new FTP_ClientException(line ?? "Server closed connection.");
			}
        }
        ...
        public string GetCurrentDir()
        {
            ...
            return r.ReadWord() ?? string.Empty;
        }
        ...
        public FTP_ListItem[] GetList(string? path)
        {
            ...
            ArgumentNullException.ThrowIfNull(m_pExtCapabilities);
            ArgumentNullException.ThrowIfNull(m_pDataConnection);
            ...
            #region LIST
                ...
                while(true){
                    ...
                    if(DateTime.TryParseExact(r.ReadWord() + " " + r.ReadWord(),winDateFormats,System.Globalization.DateTimeFormatInfo.InvariantInfo,System.Globalization.DateTimeStyles.None,out _)){
                        listingType = "win";
                    }
                    ...
                    try{
                        ...
                        // Unix listing
                            ...
                            r = new StringReader(line);
                            string attributes = r.ReadWord() ?? string.Empty;
                            r.ReadWord();
                            r.ReadWord();
                            r.ReadWord();
                            long size = Convert.ToInt64(r.ReadWord());                
        ...
        public void DeleteFile(string path)
        {
			...
			if(reply == null || !reply.StartsWith("250")){
				throw new FTP_ClientException(reply ?? "Server closed connection.");
			}
        }
        ...
        public void Rename(string fromPath,string toPath)
        {
			...
			string? reply = ReadLine();
			if(reply == null || !reply.StartsWith("350")){
				throw new FTP_ClientException(reply ?? "Server closed connection.");
			}
        ...
        public void CreateDirectory(string path)
        {
			...
			string? reply = ReadLine();
			if(reply == null || !reply.StartsWith("257")){
				throw new FTP_ClientException(reply ?? "Server closed connection.");
			}
        }
        ...
        {    
            ArgumentNullException.ThrowIfNull(m_pDataConnection);
            ArgumentNullException.ThrowIfNull(m_pDataConnection.LocalEndPoint);
            ...
            // If we reach so far PORT command didn't suceed.
            throw new FTP_ClientException(response?[0] ?? "Failed to establish data connection.");
        }
			...
			*/
            ...
            if(line != null && line.StartsWith("220")){
                ...
                while(line.StartsWith("220-")){
                    line = ReadLine();
                    if(line == null) {
                        throw new FTP_ClientException("Server closed connection.");
                    }
