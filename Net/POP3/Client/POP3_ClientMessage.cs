            ...
            private void DeleReadResponseCompleted(SmartStream.ReadLineAsyncOP op)
            {
                try{
                    ...
                    // Operation succeeded.
                        ...
                        string response = op.LineUtf8 ?? "";

                        // Log
                        m_pPop3Client.LogAddRead(op.BytesInBuffer,response);
                                            
                        // Server returned success response.
                        if(string.Equals(response.Split(new char[]{' '},2)[0],"+OK",StringComparison.InvariantCultureIgnoreCase)){
                            m_pOwner.m_IsMarkedForDeletion = true;
                            SetState(AsyncOP_State.Completed);
                        }
                        // Server returned error response.
                        else{
                            m_pException = new POP3_ClientException(response);
                            SetState(AsyncOP_State.Completed);
                        }
                    }
                }
