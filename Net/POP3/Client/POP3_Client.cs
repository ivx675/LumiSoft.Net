	...
	public class POP3_Client : TCP_Client
	{
        ...
        public class CapaAsyncOP : IDisposable,IAsyncOP
        {
            ...
            internal bool Start(POP3_Client owner)
            {
                ...
                ArgumentNullException.ThrowIfNull(owner.TcpStream);
            ...
            {
                try{
                    ArgumentNullException.ThrowIfNull(m_pPop3Client);
                    ArgumentNullException.ThrowIfNull(m_pPop3Client.TcpStream);
            ...
            private void CapaReadResponseCompleted(SmartStream.ReadLineAsyncOP op)
            {
                try{
                    ...
                    // Operation succeeded.
                        ...
                        ArgumentNullException.ThrowIfNull(op.LineUtf8);
            ...
            private void ReadMultiLineResponseLineCompleted(SmartStream.ReadLineAsyncOP op)
            {
                try{
                    ArgumentNullException.ThrowIfNull(m_pPop3Client);
                    ArgumentNullException.ThrowIfNull(m_pResponseLines);
            ...
            private void StlsReadResponseCompleted(SmartStream.ReadLineAsyncOP op)
            {
                try{
                    ...
                    // Operation succeeded.
                        ...
                        // Log
                        m_pPop3Client.LogAddRead(op.BytesInBuffer,responseLine);
                                            
                        // Server returned success response.
                        if(string.Equals(responseLine.Split(new char[]{' '},2)[0],"+OK",StringComparison.InvariantCultureIgnoreCase)){                        
                            // Log
                        ...
                        // Server returned error response.
                            ...
                            m_pException = new POP3_ClientException(responseLine);
                            SetState(AsyncOP_State.Completed);
                        }
                    }
                }
                ...
                try{
                    ...
                    // Operation succeeded.
                        ...
                        // Continue authenticating.
                            ...
                            byte[] serverResponse = Convert.FromBase64String(responseLine.Split(new char[]{' '},2)[1]);
        ...
        public class RsetAsyncOP : IDisposable,IAsyncOP
        {
            ...
            private void RsetReadResponseCompleted(SmartStream.ReadLineAsyncOP op)
            {
                try{
                    ArgumentNullException.ThrowIfNull(m_pPop3Client);
                    ArgumentNullException.ThrowIfNull(m_pPop3Client.m_pMessages);
            ...
            private void ReadMultiLineResponseLineCompleted(SmartStream.ReadLineAsyncOP op)
            {
                try{
                    ...
                    // Operation succeeded.
                        ...
                        // We got respone terminator(.).
                        else if(string.Equals(responseLine,".",StringComparison.InvariantCultureIgnoreCase)){
                            m_pPop3Client.m_pExtCapabilities.Clear();
                            m_pPop3Client.m_pExtCapabilities.AddRange(m_pResponseLines);
                        ...
                        // We got response line.
                            ...
                            m_pResponseLines.Add(responseLine);
                        }
                    }
                }
                catch(Exception x){
        ...
        private void ReadServerGreetingCompleted(SmartStream.ReadLineAsyncOP op,CompleteConnectCallback connectCallback)
        {
            ...
            try{
                ...
                // Greeting reading succeded.
                    ...
                    // Log.
                    LogAddRead(op.BytesInBuffer,responseLine);

                    // POP3 server accepted connection, get greeting text.
                    if(responseLine.StartsWith("+OK",StringComparison.InvariantCultureIgnoreCase)){
                        m_GreetingText = responseLine.Substring(3).Trim();

			            // Try to read APOP hash key, if supports APOP.
				        if(responseLine.IndexOf("<") > -1 && responseLine.IndexOf(">") > -1){
					        m_ApopHashKey = responseLine.Substring(responseLine.IndexOf("<"),responseLine.LastIndexOf(">") - responseLine.IndexOf("<") + 1);
				        }
                    }
                    // POP3 server rejected connection.
                    else{
                        error = new POP3_ClientException(responseLine);
                    }
                }
            }
            catch(Exception x){
