...
namespace LumiSoft.Net.SMTP.Relay
{
    ...
    public class Relay_Session : TCP_Session
    {
        ...
        private class Relay_Target
        {
            ...
            /// <param name="userName">Target host user name. Value null means no authentication.</param>
            /// <param name="password">Target host password. Value null means no authentication.</param>
        ...
        private async ValueTask StartRelaying()
        {
            ...
            // Try SMTP relay targets one by one; stop at the first IP that accepts a connection.
            while(m_pTargets.Count > 0){
                ...
                try{
                    await m_pSmtpClient.ConnectAsync(
                        new IPEndPoint(m_pLocalBindInfo.IP,0),
                        new[]{m_pActiveTarget!.Target.Address},
                        m_pActiveTarget.Target.Port,
                        false,null
                    );
            ...
            if(!m_pSmtpClient.IsConnected){
                LogText("No SMTP relay targets accepted connection; all IPs unreachable.");
                Dispose(null);
            ...
            try{
                ...
                await m_pSmtpClient.EhloHeloAsync(hostName,CancellationToken.None);
                if(!m_pSmtpClient.IsSecureConnection && ((m_pServer.UseTlsIfPossible && IsTlsSupported()) || m_pActiveTarget!.SslMode == SslMode.TLS)){
                    await m_pSmtpClient.StartTlsAsync(null,CancellationToken.None);
                    await m_pSmtpClient.EhloHeloAsync(hostName,CancellationToken.None);
                }
                if(!string.IsNullOrEmpty(m_pActiveTarget!.UserName)){
                    await m_pSmtpClient.AuthAsync(m_pSmtpClient.AuthGetStrongestMethod(m_pActiveTarget.UserName,m_pActiveTarget.Password ?? ""),CancellationToken.None);
                }
                await m_pSmtpClient.MailFromAsync(
                    this.From,
                    messageSize,
                    IsDsnSupported() ? m_pRelayItem.DSN_Ret : SMTP_t_DSN_Ret.NotSpecified,
                    IsDsnSupported() ? m_pRelayItem.EnvelopeID : null,
                    CancellationToken.None
                );
                await m_pSmtpClient.RcptToAsync(
                    this.To,
                    IsDsnSupported() ? m_pRelayItem.DSN_Notify : SMTP_t_DSN_Notify.NotSpecified,
                    IsDsnSupported() ? m_pRelayItem.OriginalRecipient : null
                );
                await m_pSmtpClient.SendMessageAsync(m_pRelayItem.MessageStream,true,CancellationToken.None);
        ...
        public int ExpectedTimeout
        {
            get{
                ...
                if(this.TcpStream != null){
                    lastActivity = this.TcpStream.LastActivity.Ticks;
                }

                return (int)(m_pServer.SessionIdleTimeout - ((DateTime.Now.Ticks - lastActivity) / 10000));
            }
        }
        ...
        /// <summary>
        /// Gets user data what was provided to Relay_Queue.QueueMessage method.
        /// </summary>
        /// <exception cref="ObjectDisposedException">Is raised when this object is disposed and this property is accessed.</exception>
