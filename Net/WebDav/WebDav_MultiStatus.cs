...
namespace LumiSoft.Net.WebDav
{
    ...
    public class WebDav_MultiStatus
    {
        ...
        internal static WebDav_MultiStatus Parse(Stream stream)
        {
            ...
            XmlNode? multiStatus = response.ChildNodes.Count > 1 ? response.ChildNodes[1] : null;

            // Invalid response.
            if(multiStatus == null || !string.Equals(multiStatus.NamespaceURI + multiStatus.LocalName,"DAV:multistatus",StringComparison.InvariantCultureIgnoreCase)){
            ...
            // Parse responses.
            foreach(XmlNode responseNode in multiStatus.ChildNodes){
