...
namespace LumiSoft.Net.Media
{
    ...
    public class AudioOut : IDisposable
    {
        ...
        private class WaveOut
        {
            ...
            private delegate void waveOutProc(IntPtr hdrvr,int uMsg,IntPtr dwUser,IntPtr dwParam1,IntPtr dwParam2);
            ...
            private class PlayItem
            {
                ...
                public PlayItem(IntPtr wavDevHandle,int dataSize)
                {
                    m_WavDevHandle = wavDevHandle;
                    m_DataSize     = dataSize;

                    m_pData = new byte[dataSize];
                    m_DataHandle = GCHandle.Alloc(m_pData,GCHandleType.Pinned);
                ...
                public void Dispose()
                {
                    WavMethods.waveOutUnprepareHeader(m_WavDevHandle,m_HeaderHandle.AddrOfPinnedObject(),Marshal.SizeOf(m_Header));
                ...
                #region method Prepare

                public void Prepare(byte[] audioData,int offset,int count)
                {
                    Array.Copy(audioData,offset,m_pData,0,count);

                    int result = WavMethods.waveOutUnprepareHeader(m_WavDevHandle,m_HeaderHandle.AddrOfPinnedObject(),Marshal.SizeOf(m_Header));
                    
                    result = WavMethods.waveOutPrepareHeader(m_WavDevHandle,m_HeaderHandle.AddrOfPinnedObject(),Marshal.SizeOf(m_Header));
                ...
                {
                    get{
                        if(m_HeaderHandle.Target == null) {
                            throw new Exception("Wave header is null.");
                        }

                        return (WAVEHDR)m_HeaderHandle.Target; 
                     }
                }
            ...
            private int                        m_BufferSize    = 320;
            private IntPtr                     m_pWavDevHandle = IntPtr.Zero;
            private int                        m_BlockSize     = 0;
            private bool                       m_IsBuffering   = true;
            private Queue<PlayItem>            m_pFreeBuffer;
            private Dictionary<long,PlayItem>  m_pInUseBuffer;
            ...
            /// <param name="bufferSize">Play buffer size in bytes.</param>
            ...
            public WaveOut(AudioOutDevice outputDevice,int samplesPerSec,int bitsPerSample,int channels,int bufferSize)
            {
                ...
                m_pFreeBuffer = new Queue<PlayItem>();
                for(int i=0;i<10;i++){
                    m_pFreeBuffer.Enqueue(new PlayItem(m_pWavDevHandle,bufferSize));
                }
                m_pInUseBuffer = new Dictionary<long, PlayItem>();
            }
            ...
            public void Dispose()
            {
                ...
                try{
                    ...
                    foreach(PlayItem item in m_pFreeBuffer.ToArray()){
                        item.Dispose();
                    }
                    foreach(PlayItem item in m_pInUseBuffer.Values){
                        item.Dispose();
                    }
            ...
            /// <param name="dwParam1">Message parameter.</param>
            /// <param name="dwParam2">Message parameter.</param>
            private void OnWaveOutProc(IntPtr hdrvr,int uMsg,IntPtr dwUser,IntPtr dwParam1,IntPtr dwParam2)
            {   
                // NOTE: MSDN warns, we may not call any wav related methods here.

                try{
                    if(uMsg == WavConstants.MM_WOM_DONE){
                        PlayItem playItem = m_pInUseBuffer[dwParam1.ToInt64()];
                        m_pInUseBuffer.Remove(dwParam1.ToInt64());
                        m_pFreeBuffer.Enqueue(playItem);
            ...
            public void Play(byte[] audioData,int offset,int count)
            {
                ...
                if(count > m_BufferSize){
                    throw new ArgumentException("Audio data is bigger than audio Buffer.");
                }

                if(m_pFreeBuffer.Count == 0){
                    return;
                }

                // Buffer empty, start buffering.
                if(m_pInUseBuffer.Count == 0){
                    m_IsBuffering = true;

                    WavMethods.waveOutPause(m_pWavDevHandle);
                }
                
                if(m_IsBuffering && m_pFreeBuffer.Count == 1){
                    m_IsBuffering = false;

                    WavMethods.waveOutRestart(m_pWavDevHandle);
                }

                PlayItem playItem = m_pFreeBuffer.Dequeue();
                m_pInUseBuffer.Add(playItem.HeaderHandle.AddrOfPinnedObject().ToInt64(),playItem);
                playItem.Prepare(audioData,offset,count);

                int result = WavMethods.waveOutWrite(m_pWavDevHandle,playItem.HeaderHandle.AddrOfPinnedObject(),Marshal.SizeOf(playItem.Header));
                if(result != MMSYSERR.NOERROR){
                    throw new Exception("Failed write into wav device, error: " + result.ToString() + ".");
                }
            }
            ...
            /// <summary>
            /// Gets number of audio frames in play buffer.
            /// </summary>
            public int AudioFramesInBuffer
            {
                get{ return m_pInUseBuffer.Count; }
            }
            ...
            /// <summary>
            /// Is raised when audio out has completed playing audio frame.
            /// </summary>
            public event EventHandler? AudioFramePlayed = null;

            /// <summary>
            /// Raises <b>AudioFramePlayed</b> event.
            /// </summary>
            private void OnAudioFramePlayed()
            {
                if(this.AudioFramePlayed != null){
                    this.AudioFramePlayed(this,new EventArgs());
                }
            }
        ...
        [Obsolete("Use AudioOut(AudioOutDevice device,AudioFormat format,int bufferSize) instead.")]
        public AudioOut(AudioOutDevice device,AudioFormat format) : this(device,format,20)
        {
        }       
        ...
        [Obsolete("Use AudioOut(AudioOutDevice device,int samplesPerSec,int bitsPerSample,int channels,int bufferSize) instead.")]
        public AudioOut(AudioOutDevice device,int samplesPerSec,int bitsPerSample,int channels) : this(device,samplesPerSec,bitsPerSample,channels,20)
        {            
        }
        ...
        /// <exception cref="ArgumentNullException">Is raised when <b>device</b> or <b>format</b> is null reference.</exception>
        public AudioOut(AudioOutDevice device,AudioFormat format,int audioFrameSize)
        {
            if(device == null){
            ...
            if(audioFrameSize < 1){
                throw new ArgumentException("Argument 'audioFrameSize' value must be > 0.","audioFrameSize");
            }
            ...
            m_BufferSize = (m_pAudioFormat.SamplesPerSecond / (1000 / audioFrameSize)) * (m_pAudioFormat.BitsPerSample / 8);

            m_pWaveOut = new WaveOut(device,format.SamplesPerSecond,format.BitsPerSample,format.Channels,m_BufferSize);
        }
        ...
        /// <exception cref="ArgumentNullException">Is raised when <b>device</b> is null reference.</exception>
        /// <exception cref="ArgumentException">Is raised when any of the arguments has invalid value.</exception>
        public AudioOut(AudioOutDevice device,int samplesPerSec,int bitsPerSample,int channels,int audioFrameSize)
        {
            if(device == null){
                throw new ArgumentNullException("device");
            }
            ...
            m_pWaveOut = new WaveOut(device,samplesPerSec,bitsPerSample,channels,m_BufferSize);
            m_pWaveOut.AudioFramePlayed += M_pWaveOut_AudioFramePlayed;
        }       
        ...
        private void M_pWaveOut_AudioFramePlayed(object? sender,EventArgs e)
        {
            OnAudioFramePlayed();
        }
        ...
        public void Write(byte[] buffer,int offset,int count)
        {
            ...
            if(count > m_BufferSize){
                throw new ArgumentOutOfRangeException("count","Argument 'count' can't be bigger than buffer size." );
            }
        ...
        /// <summary>
        /// Gets Audio frame size in bytes.
        /// </summary>
        /// <exception cref="ObjectDisposedException">Is raised when this class is Disposed and this property is accessed.</exception>
        public int AudioFrameBytes
        {
            get{
                if(m_IsDisposed){
                    throw new ObjectDisposedException(this.GetType().Name);
        ...
        public int AudioFramesInBuffer
        {
            get{ return m_pWaveOut.AudioFramesInBuffer; }
        }
