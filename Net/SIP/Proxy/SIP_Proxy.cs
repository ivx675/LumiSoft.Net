            ...
            foreach(SIP_ProxyHandler handler in this.Handlers){
                try{
                    ...
                    if(!handler.IsReusable){
                        var hInstance =Activator.CreateInstance(handler.GetType());
                        if(hInstance == null){
                            throw new InvalidOperationException("Failed to create instance of proxy handler.");
                        }
                        h = (SIP_ProxyHandler)hInstance;
                    }
