...
namespace LumiSoft.Net.SIP.Stack
{
    ...
    public class SIP_RequestSender : IDisposable
    {
        ...
        {     
            ArgumentNullException.ThrowIfNull(m_pTransaction);
            ArgumentNullException.ThrowIfNull(e.ClientTransaction);
                    ...
                    // Try to authorize challenges.
                        ...
                        */
                        if(request.CSeq == null) {
                            throw new SIP_ParseException("Request does not contain required CSeq header field.");
                        }
                ...
                ThreadPool.QueueUserWorkItem(new WaitCallback(delegate(object? state){
                    lock(m_pLock){
                        ...
                        */
                        ...
                        SIP_Uri uri;
                        var topMostRoute = m_pRequest.Route.GetTopMostValue();
                        if(isStrictRoute){
                            uri = (SIP_Uri)m_pRequest.RequestLine.Uri;
                        }
                        else if(topMostRoute != null){
                            uri = (SIP_Uri)topMostRoute.Address.Uri;
                        }
                        else{
                            uri = (SIP_Uri)m_pRequest.RequestLine.Uri;
                        }
        ...
        private void SendToFlow(SIP_Flow flow,SIP_Request request)
        {
            ...
            if(request.From == null) {
                throw new ArgumentException("Request 'From' header field value is missing.");
            }
