...
namespace LumiSoft.Net.SIP.Stack
{
    ...
    public class SIP_Flow : IDisposable
    {
        ...
        internal SIP_Flow(SIP_Stack stack,TCP_ServerSession session)
        {
            ...
            ArgumentNullException.ThrowIfNull(session.LocalEndPoint);
            ArgumentNullException.ThrowIfNull(session.RemoteEndPoint);
                    ...
                    if(m_pLocalPublicEP != null){
                        ...
                        try{
                            ...
                            optionsTransaction.ResponseReceived += new EventHandler<SIP_ResponseReceivedEventArgs>(delegate(object? s,SIP_ResponseReceivedEventArgs e){
                                ...
                                IPEndPoint publicEP;
                                if(via != null){
                                    publicEP = new IPEndPoint(via.Received!,via.RPort > 0 ? via.RPort : this.LocalEP.Port);
                                }
                                else{
                                    publicEP = new IPEndPoint(this.LocalEP.Address,this.LocalEP.Port);
                                }
        ...
        public DateTime LastActivity
        {
            get{
                ...
                if(m_Transport == SIP_Transport.TCP || m_Transport == SIP_Transport.TLS){
                    return m_pTcpSession?.LastActivity ?? DateTime.MinValue;
                }
                else{
