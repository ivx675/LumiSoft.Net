...
namespace LumiSoft.Net.MIME
{
    ...
    public class MIME_b_Provider
    {
        ...
        public MIME_b Parse(MIME_Entity owner,SmartStream stream,MIME_h_ContentType defaultContentType)
        {
            ...
            var methodParse = bodyType.GetMethod("Parse",BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.FlattenHierarchy);
            var retVal = methodParse?.Invoke(null,new object[]{owner,defaultContentType,stream});
            if(retVal != null){
                return (MIME_b)retVal;
            }
            else{
                throw new ParseException("Failed to parse MIME entity body " + mediaType + ".");
            }
        }
